...
`feedback.md` shows the review loop: F-01 was left on the site, applied in the `feedback` phase (spec, tests and design hook changed together) and resolved with a note; F-02 was a question answered without an edit. F-03 is the same loop on a wireframe: file-level anchor, screen edited, resolved with a note.
